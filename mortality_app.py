...
# TODO 초기값으로 돌아가는 reset 버튼 추가, 키보드 좌우키 등으로 초미세조정 가능하게 하기(한 bounds의 1/1000정도?)
#  파라미터 창 좀 크게 만들기, x* 실시간으로 나오게 하기, 
...
def create_gui_app():
    ...
    for i, p in enumerate(param_labels):
        ...
        label = tk.Label(row_frame, text=f'Parameter {p}:')
        ...
        btn = tk.Button(row_frame, text="Set", command=lambda p=p: update_from_entry(p))
